...
//   onToggleExplanation,
// }) {
//   const { deadlock_detected, cycle_path, explanation } = graphData;
...
//             {deadlock_detected ? 'Deadlock Detected' : 'No Deadlock — System is Safe'}
//           </div>
//           {deadlock_detected && cycle_path?.length > 0 && (
//             <div className="status-cycle">
//               <span className="cycle-label">Cycle path:</span>
//               <div className="cycle-path">
//                 {cycle_path.map((node, i) => (
//                   <React.Fragment key={i}>
//                     <span className="cycle-node">{node}</span>
//                     {i < cycle_path.length - 1 && (
...
//         <button
//           className={`ra-btn ${showSteps ? 'ra-btn--active' : ''}`}
//           onClick={onToggleSteps}
//         >
//           <span>🔍</span>
//           {showSteps ? 'Hide' : 'Show'} Detection Steps
//         </button>

//         <button
//           className={`ra-btn ${showExplanation ? 'ra-btn--active' : ''}`}
//           onClick={onToggleExplanation}
//         >
//           <span>📘</span>
//           {showExplanation ? 'Hide' : 'Explain'} Result
//         </button>
//       </div>
...
// components/ResultPanel.js — Extended with safe sequence + multi-instance deadlock
import React from 'react';
import './ResultPanel.css';
